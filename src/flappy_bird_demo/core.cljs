...
(defn reset-state [_ cur-time]
  (-> starting-state
      (update-in [:pillar-list]
                 (fn [pls] (map #(assoc % :start-time cur-time) pls)))
      (assoc
        :start-time cur-time
...
(defn curr-pillar-pos
  [cur-time {:keys [pos-x start-time] }]
  (->> (- cur-time start-time)
       (translate pos-x velo-x)))
...
(defn update-pillars [{:keys [pillar-list cur-time] :as st}]
  (let [pillars-with-pos (-> #(assoc % :cur-x (curr-pillar-pos cur-time %))
                             (map pillar-list))
        pillars-in-world (sort-by
                           :cur-x
                           (-> #(> (:cur-x %) (- pillar-width))
                               (filter pillars-with-pos)))]
    (assoc st
      :pillar-list
...
(defn update-flappy
  [{:keys [time-delta initial-vel flappy-y jump-count] :as st}]
  (if (pos? jump-count)
...
(defn fibo
  "Returns the nth positive prime"
  [n]
  (loop [i (int 0) a (int 1) b (int 0)]
    (if (= i n)
      a
      (recur (inc i) (+ a b) a))))

(defn score [{:keys [cur-time start-time] :as st}]
  (let [score (- (.abs js/Math (floor (/ (- (* (- cur-time start-time) velo-x) 544)
                                         pillar-spacing)))
                 4)]
    (->> (nth-prime score)
         (if (neg? score) 0)
...
(defn border
  [{:keys [cur-time] :as state}]
  (-> state
      (assoc :border-pos (mod (translate 0 velo-x cur-time) 23))))

(defn pillar-offset
  [{:keys [cur-time]} {:keys [gap-top] :as p}]
  (assoc p
  ...
  (sab/html [:div.board { :onMouseDown (fn [e]
             ...
             (if-not timer-running
               [:a.start-button {:onClick #(start-game)}
                (if (< 1 jump-count) "WORK-ART" "START BROH")]
               [:span])
             [:div (map pillar pillar-list)]
             [:div.flappy {:style {:top (px flappy-y)}}]
             [:div.scrolling-border {:style { :background-position-x (px border-pos)}}]]))

(let [node (selid "board-area")]
  (defn renderer
    [full-state]
